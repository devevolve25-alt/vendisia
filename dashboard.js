//atualizado para exibição de agendamentos
...
async function atualizarDashboard(salaoId) {
    try {
        ...
        const inicioMes = `${agora.getFullYear()}-${String(agora.getMonth() + 1).padStart(2, '0')}`; // "2026-02"
        ...
        const { data: movs } = await supabaseClient
            ...
            .ilike('data_movimentacao', `%${inicioMes}%`);

        // 3. BUSCA AGENDAMENTOS (Correção assertiva usando ILIKE para ignorar fuso/máscara)
        const { data: ags } = await supabaseClient
            .from('agendamentos')
            .select('id, data_hora_inicio') 
            .eq('estabelecimento_id', salaoId)
            .ilike('data_hora_inicio', `%${hojeISO}%`);

        const { data: agsMes } = await supabaseClient
            .from('agendamentos')
            .select('id')
            .eq('estabelecimento_id', salaoId)
            .ilike('data_hora_inicio', `%${inicioMes}%`);
        ...
        document.getElementById('ag-man-hoje').innerText = ags?.length || 0;
        ...
        document.getElementById('top-barbeiro').innerText = Object.keys(ranking).reduce((a, b) => ranking[a] > ranking[b] ? a : b, "---");
